  ...
  <div>
    <div class="postList">
      <el-scrollbar style="height: 600px">
        <div
          v-for="(post, index) in this.postInfo"
          :key="post.postId"
          class="postItem"
        >
            ...
            <el-card shadow="hover" v-show="postShow">
              <div class="clearfix" style="text-align: left">
                ...
                <el-button
                  ...
                  type="text"
                  style="padding:0px;font-size: 18px;color: #606266"
                >
                  {{ post.postTitle }}
                </el-button>

                <el-button
                  style="float: right; padding: 0px 5px 0 5px; margin: 0"
                  type="text"
                ...
                <el-button
                  style="float: right; padding: 0px 5px 0 5px; margin: 0 "
                  type="text"
                  @click="collectPost(index)"
                  v-show="collectindex[index]"
                >
                ...
                <el-button
                  style="float: right; padding: 0px 5px 5px 5px; width: 50px"
                  type="text"
                  @click="likePost(index)"
                  v-show="likeindex[index]"
                >
                ...
                <span style="float: right;" v-if="post.label != 'Normal'"
                  >{{ post.label }}
                </span>
              </div>
            </el-card>
  ...
  methods: {
    async likePost(index) {
      if (this.likeindex[index] == true) {
        ...
        await likePost(this.postInfo[index].postId).then(res => {
    ...
    async collectPost(index) {
      if (this.collectindex[index] == true) {
        ...
        await collectPost(data).then(res => {
          if (res.data.code == 200) {
            this.$notify({
              title: "收藏成功",
              position: "top-left",
    ...
    async reportPost(index) {
      await reportPost(this.postInfo[index].postId).then(res => {
        if (res.data.code == 200) {
          this.$notify({
            title: "举报成功",
            position: "top-left",
    ...
    async searchInfoChanged() {
      ...
      await getPostBySearch(this.computedSearchInfo).then(res => {
        this.postInfo = [];
        for (var i in res.data.data) {
    ...
    async changePostData() {
      ...
      if (this.computedInfo1 == 1) {
        ...
        console.log(this.computedInfo2);
        await getRankByDay(this.computedInfo2).then(res => {
          this.postInfo = [];
          for (var i in res.data.data) {
            this.postInfo.push(res.data.data[i]);
            this.likeindex[i] = true;
        ...
        await getPostByZoneId(this.computedInfo2).then(res => {
          this.postInfo = [];
          console.log(res.data);
    ...
    async followZone() {
      ...
      await followZone(data).then(res => {
        if (res.data.code == 200) {
          this.$message({
            message: "关注成功",
            type: "success",
