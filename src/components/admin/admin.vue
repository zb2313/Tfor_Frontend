<template>
  <div>
    <el-card v-for="(post,index) in this.contentlist" :key="post.contentId" class="contentCard">
      ...
      <el-button type="primary" @click="check(post.contentId)" size="medium" style="float:right;margin-left:10px">
        check
      </el-button>
      <el-button type="primary" @click="setState(post.contentId, value[index])" size="medium" style="float:right;" >
        set
      </el-button>
<!--      <el-button type="primary" @click="setNotnormal(post.contentId)" size="medium"-->
<!--                 style="float:right; margin-right: 5px;">UnNormal-->
<!--      </el-button>-->
      <el-select v-model="value[index]" placeholder="请选择" style="width: 100px;float:right;margin-bottom: 10px">
        <el-option
...
export default {
  ...
  methods: {
    ...
    setState(contentId, label) {
      ...
      this.$axios
          .put('http://121.5.137.205:8081/api/admin/setPostLabel?contentId=' + contentId + '&label=' + label)
    },
    getdata() {
      this.$axios
