...
export default {
  ...
  methods: {
    ...
    reFresh() {
      getbyPostId(this.postID).then(res => {
    ...
    async getData() {
      ...
      await getbyPostId(this.postID).then(res => {
        this.postInfo = res.data.data;
        console.log(this.postInfo);
        getUserImg(this.postInfo.userId).then(res => {
          this.userImage = res.data.data;
          // console.log('帖子用户头像',res.data)
        });
      });

      getPostImgs(this.postID).then(res => {
        this.imgs = res.data.data;
        console.log("帖子图片", this.imgs);
      });
      getCommentPost(this.postID).then(res => {
        this.comments = res.data.data;
        console.log("评论", res.data);
        this.getCommentImgs();
      });

      getRankByDay(5).then(res => {
