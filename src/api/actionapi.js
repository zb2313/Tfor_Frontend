...
const baseurl = "http://121.5.137.205:8081/api/action/"
...
export const cancelCollectPost = (userId, postId) => {
    let url = baseurl + "cancelCollectPost?userId=" + userId + "&postId=" + postId
    console.log(url)
    return axios.delete(
        `${url}`
    )
...
export const cancelFollowUser = (userId1, userId2) => {
    let url = baseurl + "cancelFollowUser?userId1=" + userId1 + "&userId2=" + userId2
    console.log(url)
    return axios.delete(
        `${url}`
    )
